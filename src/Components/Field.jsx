  ...
  return (
    <div className="container my-5" style={{color:(props.mode === 'dark')?'white':'black'}}>
      ...
      <div className="container my-3">
        <button onClick={buttonHandler} className="btn btn-primary mx-2">
          Press me Here to UpperCase
        </button>
        <button onClick={buttonloHandler} className="btn btn-primary mx-2">
          Press me Here to LowerCase
        </button>
        <button onClick={upgradeTextHandler} className="btn btn-primary mx-2">
          Press me Here to Upgrade Text
        </button>
      </div>
      <h3>total words {mainText.split(" ").length - 1}</h3>
      <h3>Total reading Time: {0.008 * mainText.split(" ").length}</h3>
    </div>
  );
}
